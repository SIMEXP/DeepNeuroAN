...
class ChannelwiseConv3D(tf.keras.layers.Layer):
    ...
    def __init__(self, filters=1, kernel_size=(1, 1, 1), dilation_rate=(1, 1, 1), padding='SAME', strides=(1, 1, 1),
                 **kwargs):
...
def encode_block_channelwise(x, n_filters, name, params_conv, params_layer):
    '''''
    ...
    x = ChannelwiseConv3D(name=name + "_conv", filters=n_filters, **params_conv)(x)
    ...
    x = tf.keras.layers.MaxPool3D(pool_size=params_layer["pool_size"], padding=params_layer["padding"], name=name + "_maxpool")(x)
...
def rigid_concatenated(kernel_size=(3, 3, 3)
    ...
    params_conv = dict(kernel_size=kernel_size, kernel_initializer='glorot_uniform', activation=activation, padding=padding)
    params_dense = dict(kernel_initializer='glorot_uniform', activation=activation)
    ...
    input = tf.keras.Input(shape=(None, None, None, 2))
    # encoder part
    for i in range(n_encode_layers):
        features = input if i == 0 else features
        fmps = filters * growth_rate**i
        x = encode_block_channelwise(features, fmps, "encode%02d" % i, params_conv, params_layer)
    regression = tf.keras.layers.Flatten()(features)

    # regression
    for i in range(n_regression_layers):
        unts = units // growth_rate ** i
        regression = regression_block(regression, unts, "regress%02d" % i, params_dense, params_layer)
    output = tf.keras.layers.Dense(units=7
                                   , kernel_initializer=params_dense["kernel_initializer"]
